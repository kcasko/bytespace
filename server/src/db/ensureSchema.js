...
export async function ensureOperationalSchema() {
  ...
  await query('CREATE INDEX IF NOT EXISTS notifications_user_read_created_idx ON notifications(user_id, read_at, created_at DESC)');
  await query('CREATE INDEX IF NOT EXISTS notifications_user_created_idx ON notifications(user_id, created_at DESC)');
  await query('CREATE INDEX IF NOT EXISTS notifications_actor_user_id_idx ON notifications(actor_user_id)');
